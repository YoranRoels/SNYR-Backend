...
public class StudentReader implements MessageBodyReader<Student>{
    ...
    public Student readFrom(Class<Student> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, String> httpHeaders, InputStream entityStream) throws IOException, WebApplicationException {
              try(JsonReader in = Json.createReader(entityStream)){
                  JsonObject jsonStudent = in.readObject();
                  Student student = new Student();
                student.setVoornaam(jsonStudent.getString("surname","test2"));
                student.setAchternaam(jsonStudent.getString("firstname"));
                student.setEvanumber(jsonStudent.getInt("currenteva"));
                 System.out.println(jsonStudent.getString("surname"));
                System.out.println(jsonStudent.getInt("currenteva"));
